...
def consolidate_csv_as_df(glob_path, infer_run=False):
    ...
    for csv_file in glob_path:
        ...
        if infer_run:
            matched_run_num = re.search('(\d+)_(\d+).*', str(csv_file))
            if matched_run_num:
                run_num = matched_run_num.group(1)
                print(f'Adding run number {run_num}')
                data_frames[-1]['run'] = int(run_num)
                iteration_num = matched_run_num.group(2)
                print(f'Adding iteration number {iteration_num}')
                data_frames[-1]['iteration'] = int(iteration_num)
                print(f'New dataframe length {len(data_frames[-1])}')

    return pd.concat(data_frames)



def consolidate_learner_csv_as_df(glob_path, infer_run=False):
    """ Convenience for importing a collection of CSV files from the Q-learner
     into a single pandas dataframe.

    >>> my_df = consolidate_csv_as_df(Path('.').glob('*progress.csv'))
    ...
    for csv_file in glob_path:
        ...
        if infer_run:
            matched_run_num = re.search('run(\d+)_.*iter(\d+)_.*', str(csv_file))
            if matched_run_num:
                run_num = matched_run_num.group(1)
                print(f'Adding run number {run_num}')
                data_frames[-1]['run'] = int(run_num)
...
if __name__ == '__main__':
    ...
    gremlin_runs = consolidate_csv_as_df(Path('.').glob('*pop.csv'),
                                         infer_run=True)
