...
def async_init_evaluate(population, cluster_sim, strategy: str, create_individual=None):
    ...
    if strategy == 'immediate':
        ...
        assert(cluster_sim.num_waiting == max(0, (len(population) - cluster_sim.num_processors - 1))), f"Expected {(len(population) - cluster_sim.num_processors - 1)} individuals waiting in the queue, but found {cluster_sim.num_waiting}."
...
def async_evaluate(next_individual, cluster_sim):
    ...
    The sweet_output of this function depends on what is currently
...
class TwoBasinProblem(ScalarProblem):
    ...
    def evaluate(self, individual):
        ...
        return y_offset + self.a*self.basin_1.evaluate(individual) + self.b*self.basin_2.evaluate(individual)
...
def get_standard_function(name: str, dimensions: int):
    ...
    if name == 'two-basin-a-better':
        return TwoBasinProblem(a=10, b=1, dimensions=dimensions, maximize=True)
    if name == 'two-basin-b-better':
    ...
    if name == 'random-uniform':
        ...
        raise ValueError(f"Unrecognized function name '{name}'.")
